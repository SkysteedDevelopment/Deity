...
In the core engine file, it will check to see if the "slg" value is present. If it is, it will then check if you're already logged in under that UniID. If you're not logged in with that UniID, it will communicate with the Auth server and confirm that you own the UniID and have permissions to log in with it. Then, it will log you in. This happens automatically without the user knowing it (hence the terminology "soft login").
