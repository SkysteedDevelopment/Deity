...
/****** Import data with a JSON blob ******/
	public static function importWithJSON
	(
		$jsonBlob		// <str> The JSON-formatted string to update.
	)					// RETURNS <bool> TRUE if all tests passed, FALSE on failure.
	
	// static::importWithJSON($jsonBlob)
	{
		// Extract the JSON data
		$dataSet = json_decode($jsonBlob, true);
		
		// Check if the import schema is one array, or has many arrays
		if(!isset($dataSet[0]))
		{
			$dataSet = [$dataSet];
		}
		
		// Loop through each row of data to insert it
		foreach($dataSet as $dataRow)
		{
			// Check if the lookup ID already exists
			// If it does, do an update instead of an insert
			if($dataRow[static::$lookupKey])
			{
				// Need to handle special processing for JSON Updates
			}
			
			// Make sure the appropriate data is available
			//else if(static::processForm($dataRow))
			{
				// Need to handle special processing for JSON Inserts
			}
		}
	}
