...
if(!file_exists($engineDirectory . "/global-config.php"))
{
	echo "Unable to locate global-config.php. Please set the engine directory value to the proper directory.\n";
	exit;
}
...
$content = file_get_contents($engineDirectory . "/global-config.php");
...
passthru('echo "' . str_replace(array('"', '$'), array('\"', '\$'), $content) . '" > "' . $engineDirectory . "/global-config.php" . '"');
